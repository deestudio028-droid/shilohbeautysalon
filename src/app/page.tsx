  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Filter featured services (first few non-bridal services)
        setFeaturedServices(allServices.filter(s => s.category !== "Bridal Services").slice(0, 4));
        ...
        const approved = allFeedbacks.filter((f) => f.status === "Approved");
        setReviews(approved);
      } catch (err) {
    ...
    <div className="bg-[#050B1F] min-h-screen relative text-white font-sans selection:bg-[#FF2D95] selection:text-white">
      ...
      <section className="relative min-h-screen flex items-center justify-center pt-24 pb-16 overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <Image
            src="/images/bridal-silhouette.png"
            alt=""
            fill
